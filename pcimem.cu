...
__global__
void access_kernel(volatile uint8_t* map_base, int access_type, int items_count, off_t  target, off_t target_base, int type_width) {
  int verbose = 1;
  int read_result_dupped = 0;
  uint64_t read_result, prev_read_result = 0;
  for (uint32_t i = 0; i < items_count; i++) {

    volatile uint8_t* virt_addr = map_base + target + i*type_width - target_base;
    switch(access_type) {
    case 'b':
...
int main(int argc, char **argv) {
	...
	void* devicePtr = NULL;
	volatile uint8_t* d_map_base = NULL;
    ...
    if (err != cudaSuccess)
      PRINT_ERROR;
    err = cudaHostRegister((void*) map_base, map_size, cudaHostRegisterIoMemory);
    if (err != cudaSuccess)
      PRINT_ERROR;
    err = cudaHostGetDevicePointer(&devicePtr, (void*) map_base, 0);
    if (err != cudaSuccess)
      PRINT_ERROR;
    d_map_base = (volatile uint8_t*) devicePtr;
    printf("GPU Results:\n");
    access_kernel<<<1,1>>>(d_map_base, access_type, items_count, target, target_base, type_width);
    err = cudaDeviceSynchronize();
    if (err != cudaSuccess)
      PRINT_ERROR;
    err = cudaHostUnregister(map_base);
